...
# Output: diện tích
count_area_rectangle <- function(length, width) {
  acreage <- length * width
  return(acreage)
}
count_area_rectangle(3,3)
# 2. Viết function tính chu vi hình tròn
# Input: bán kính
# Output: chu vi
count_perimeter_circle <- function(r){
  cv <- 2*r*3.14
  return(cv)
}
count_perimeter_circle(5)
# 3. Viết function chuyển đổi nhiệt độ từ Celsius sang Fahrenheit
# Công thức: F = C * 9/5 + 32
cd_nhietdo <- function(c, f){
  f = c * 9/5 + 32
  return(f)
}
cd_nhietdo(32)
...
#   - Loại bỏ giá trị NA
calc_avg_grade <- function(grades) {
  grades <- na.omit(grades)
  if (any(grades < 0 | grades > 10)) {
    stop("Error")
  }
  return(mean(grades))
}
...
generate_pascal_triangle <- function(n_rows) {
  for (i in 0:(n_rows - 1)) {
    # Tạo khoảng trắng để căn lề tam giác
    padding <- paste(rep(" ", n_rows - i), collapse = "")
    
    # Tính các giá trị C(i, k)
    row_values <- sapply(0:i, function(k) choose(i, k))
    
    # In hàng
    cat(padding, row_values, "\n")
  }
}
...
classify_student <- function(score) {
  ...
  # Phân loại và tính GPA
  if (score >= 9) {
    result <- list(grade = "Excellent (Xuất sắc)", gpa = 4.0)
  } else if (score >= 8) {
    result <- list(grade = "Good (Giỏi)", gpa = 3.5)
  } else if (score >= 6.5) {
    result <- list(grade = "Fair (Khá)", gpa = 3.0)
  } else if (score >= 5) {
    result <- list(grade = "Average (Trung bình)", gpa = 2.0)
  } else {
    result <- list(grade = "Weak (Yếu)", gpa = 1.0)
  }
...
calculate_net_salary <- function(base_salary, allowance, working_days, overtime_hours, tax_rate = 0.1) {
  # Giả định 1 tháng làm việc tiêu chuẩn là 22 ngày
  daily_rate <- base_salary / 22
  hourly_overtime_rate <- (daily_rate / 8) * 1.5
  
  gross_salary <- (daily_rate * working_days) + (hourly_overtime_rate * overtime_hours) + allowance
...
# Sử dụng thử:
calculate_net_salary(base_salary = 15000000, allowance = 2000000, working_days = 20, overtime_hours = 5)
# 2. Viết function chuẩn hóa điểm thi
# Input: vector điểm thô
# Output: vector điểm chuẩn hóa (0-100)
# Công thức: (điểm - min) / (max - min) * 100
normalize_scores <- function(score_vector) {
  min_val <- min(score_vector, na.rm = TRUE)
  max_val <- max(score_vector, na.rm = TRUE)
  
  # Tránh lỗi chia cho 0 nếu tất cả điểm số bằng nhau
  if (max_val == min_val) return(rep(100, length(score_vector)))
  
  normalized <- (score_vector - min_val) / (max_val - min_val) * 100
  return(normalized)
}

# Sử dụng thử:
raw_scores <- c(5, 7, 8, 9, 10)
...
analyze_student_data <- function(df) {
  # Thống kê cơ bản cho cột điểm (score)
  summary_stats <- summary(df$score)
  
  # Tính thêm độ lệch chuẩn
  std_dev <- sd(df$score, na.rm = TRUE)
